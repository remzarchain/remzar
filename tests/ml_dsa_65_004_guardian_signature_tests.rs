...
static GUARDIAN_SIGNATURE_TEST_LOCK: OnceLock<Mutex<()>> = OnceLock::new();

fn run_serial<F>(f: F) -> TestResult
where
    F: FnOnce() -> TestResult,
{
    let lock = GUARDIAN_SIGNATURE_TEST_LOCK.get_or_init(|| Mutex::new(()));
...
fn sign_for(kp: &MlDsa65Keypair, batch: &[&[u8]]) -> Result<Vec<u8>, String> {
    ...
    GuardianSignature::sign_batch(&signing_key, batch).map_err(debug_err)
}
...
fn verify_for(kp: &MlDsa65Keypair, batch: &[&[u8]], signature: &[u8]) -> Result<(), String> {
    ...
    GuardianSignature::verify_batch(&verifying_key, batch, signature).map_err(debug_err)
}
...
fn guardian_batch_root(batch: &[&[u8]]) -> Result<[u8; 64], String> {
...
#[test]
fn guardian_001_signature_length_constant_matches_config_and_fips() -> TestResult {
    run_serial(|| {
...
#[test]
fn guardian_002_sign_empty_batch_returns_exact_signature_length() -> TestResult {
    run_serial(|| {
...
#[test]
fn guardian_003_verify_empty_batch_succeeds() -> TestResult {
    run_serial(|| {
        let kp = generate_keypair()?;
        let batch: Vec<&[u8]> = Vec::new();
        let signature = sign_for(&kp, &batch)?;
...
#[test]
fn guardian_004_sign_single_transaction_returns_exact_signature_length() -> TestResult {
    run_serial(|| {
        let kp = generate_keypair()?;
        let batch: Vec<&[u8]> = vec![b"guardian-single".as_slice()];
        let signature = sign_for(&kp, &batch)?;
...
#[test]
fn guardian_005_verify_single_transaction_succeeds() -> TestResult {
    run_serial(|| {
        let kp = generate_keypair()?;
        let batch: Vec<&[u8]> = vec![b"guardian-single".as_slice()];
        let signature = sign_for(&kp, &batch)?;
...
#[test]
fn guardian_006_verify_two_transaction_batch_succeeds() -> TestResult {
    run_serial(|| {
        let kp = generate_keypair()?;
        let batch: Vec<&[u8]> = vec![b"guardian-a".as_slice(), b"guardian-b".as_slice()];
        let signature = sign_for(&kp, &batch)?;
...
#[test]
fn guardian_007_verify_odd_three_transaction_batch_succeeds() -> TestResult {
    run_serial(|| {
        let kp = generate_keypair()?;
        let batch: Vec<&[u8]> = vec![
            b"guardian-a".as_slice(),
            b"guardian-b".as_slice(),
            b"guardian-c".as_slice(),
        ];
        let signature = sign_for(&kp, &batch)?;
...
#[test]
fn guardian_008_empty_transaction_inside_batch_succeeds() -> TestResult {
    run_serial(|| {
        let kp = generate_keypair()?;
...
#[test]
fn guardian_009_binary_transaction_with_zero_bytes_succeeds() -> TestResult {
    run_serial(|| {
        let kp = generate_keypair()?;
...
#[test]
fn guardian_010_duplicate_transactions_batch_succeeds() -> TestResult {
    run_serial(|| {
        let kp = generate_keypair()?;
        let batch: Vec<&[u8]> = vec![
            b"dup".as_slice(),
...
#[test]
fn guardian_011_wrong_verifying_key_rejects_signature() -> TestResult {
    run_serial(|| {
...
#[test]
fn guardian_012_flipped_signature_byte_is_rejected() -> TestResult {
    run_serial(|| {
        let kp = generate_keypair()?;
        let batch: Vec<&[u8]> = vec![b"a".as_slice(), b"b".as_slice()];
...
#[test]
fn guardian_013_signature_short_by_one_is_rejected() -> TestResult {
    run_serial(|| {
        let kp = generate_keypair()?;
...
#[test]
fn guardian_014_signature_long_by_one_is_rejected() -> TestResult {
    run_serial(|| {
        let kp = generate_keypair()?;
        let batch: Vec<&[u8]> = vec![b"a".as_slice()];
        let mut signature = sign_for(&kp, &batch)?;
...
#[test]
fn guardian_015_empty_signature_is_rejected() -> TestResult {
    run_serial(|| {
        let kp = generate_keypair()?;
        let batch: Vec<&[u8]> = vec![b"a".as_slice()];
...
#[test]
fn guardian_016_all_zero_exact_length_signature_is_rejected() -> TestResult {
    run_serial(|| {
        let kp = generate_keypair()?;
        let batch: Vec<&[u8]> = vec![b"a".as_slice()];
...
#[test]
fn guardian_017_repeated_byte_exact_length_signature_is_rejected() -> TestResult {
    run_serial(|| {
        let kp = generate_keypair()?;
        let batch: Vec<&[u8]> = vec![b"a".as_slice()];
...
#[test]
fn guardian_018_changed_transaction_content_is_rejected() -> TestResult {
    run_serial(|| {
        let kp = generate_keypair()?;
...
#[test]
fn guardian_019_reordered_batch_is_rejected() -> TestResult {
    run_serial(|| {
        let kp = generate_keypair()?;
        let signed_batch: Vec<&[u8]> =
            vec![b"tx-0".as_slice(), b"tx-1".as_slice(), b"tx-2".as_slice()];
...
#[test]
fn guardian_020_appended_transaction_is_rejected() -> TestResult {
    run_serial(|| {
        let kp = generate_keypair()?;
        let signed_batch: Vec<&[u8]> = vec![b"a".as_slice(), b"b".as_slice()];
...
#[test]
fn guardian_021_removed_transaction_is_rejected() -> TestResult {
    run_serial(|| {
        let kp = generate_keypair()?;
...
#[test]
fn guardian_022_inserted_duplicate_transaction_is_rejected() -> TestResult {
    run_serial(|| {
        let kp = generate_keypair()?;
        let signed_batch: Vec<&[u8]> = vec![b"a".as_slice(), b"b".as_slice(), b"c".as_slice()];
...
#[test]
fn guardian_023_empty_batch_signature_does_not_verify_nonempty_batch() -> TestResult {
    run_serial(|| {
        let kp = generate_keypair()?;
...
#[test]
fn guardian_024_nonempty_batch_signature_does_not_verify_empty_batch() -> TestResult {
    run_serial(|| {
        let kp = generate_keypair()?;
        let empty_batch: Vec<&[u8]> = Vec::new();
        let nonempty_batch: Vec<&[u8]> = vec![b"a".as_slice()];
...
#[test]
fn guardian_025_signature_verifies_manually_against_expected_merkle_root() -> TestResult {
    run_serial(|| {
        let kp = generate_keypair()?;
        let batch: Vec<&[u8]> = vec![
            b"manual-a".as_slice(),
        ...
        let signature = sign_for(&kp, &batch)?;
        let root = guardian_batch_root(&batch)?;
...
#[test]
fn guardian_026_signature_fails_manually_against_wrong_merkle_root() -> TestResult {
    run_serial(|| {
        let kp = generate_keypair()?;
        let signed_batch: Vec<&[u8]> = vec![b"a".as_slice(), b"b".as_slice()];
        let wrong_batch: Vec<&[u8]> = vec![b"a".as_slice(), b"changed-b".as_slice()];
        let signature = sign_for(&kp, &signed_batch)?;
        let wrong_root = guardian_batch_root(&wrong_batch)?;
...
#[test]
fn guardian_027_signature_fails_with_nonempty_context() -> TestResult {
    run_serial(|| {
        let kp = generate_keypair()?;
...
#[test]
fn guardian_028_signing_same_batch_twice_produces_verifiable_signatures() -> TestResult {
    run_serial(|| {
        let kp = generate_keypair()?;
...
#[test]
fn guardian_029_property_batches_of_size_zero_through_eight_verify() -> TestResult {
    run_serial(|| {
        let kp = generate_keypair()?;

        for count in 0..=8 {
            let txs = tx_vectors("guardian-prop-size", count);
...
#[test]
fn guardian_030_property_each_single_position_content_change_rejects() -> TestResult {
    run_serial(|| {
        let kp = generate_keypair()?;
        let original_txs = tx_vectors("guardian-content-change", 6);
...
#[test]
fn guardian_031_sign_rejects_batch_count_above_max_items() -> TestResult {
    run_serial(|| {
        let kp = generate_keypair()?;
...
#[test]
fn guardian_032_verify_rejects_batch_count_above_max_items() -> TestResult {
    run_serial(|| {
        let kp = generate_keypair()?;
...
#[test]
fn guardian_033_sign_rejects_item_larger_than_max_item_bytes() -> TestResult {
    run_serial(|| {
        let kp = generate_keypair()?;
...
#[test]
fn guardian_034_verify_rejects_item_larger_than_max_item_bytes() -> TestResult {
    run_serial(|| {
        let kp = generate_keypair()?;
...
#[test]
fn guardian_035_sign_rejects_batch_over_total_byte_limit() -> TestResult {
    run_serial(|| {
        let kp = generate_keypair()?;
...
#[test]
fn guardian_036_verify_rejects_batch_over_total_byte_limit() -> TestResult {
    run_serial(|| {
        let kp = generate_keypair()?;
        let valid_batch: Vec<&[u8]> = vec![b"valid".as_slice()];
        let signature = sign_for(&kp, &valid_batch)?;
...
#[test]
fn guardian_037_adversarial_signature_byte_by_byte_reassembly_verifies() -> TestResult {
    run_serial(|| {
        let kp = generate_keypair()?;
...
#[test]
fn guardian_038_adversarial_signature_fragment_swap_rejects() -> TestResult {
    run_serial(|| {
        let kp = generate_keypair()?;
        let batch: Vec<&[u8]> = vec![b"net-a".as_slice(), b"net-b".as_slice()];
        let signature = sign_for(&kp, &batch)?;
...
#[test]
fn guardian_039_load_test_sign_and_verify_many_small_batches() -> TestResult {
    run_serial(|| {
        let kp = generate_keypair()?;

        for round in 0..24 {
            let txs = tx_vectors("guardian-load-small", round % 9);
            let batch = tx_refs(&txs);
            let signature = sign_for(&kp, &batch)?;
...
#[test]
fn guardian_040_load_test_large_batch_sign_verify_and_manual_root_verify() -> TestResult {
    run_serial(|| {
        let kp = generate_keypair()?;
        let txs = tx_vectors("guardian-load-large", 128);
        let batch = tx_refs(&txs);
        let signature = sign_for(&kp, &batch)?;
        let root = guardian_batch_root(&batch)?;
...
#[test]
fn guardian_041_empty_batch_signature_verifies_manually_against_dummy_merkle_root() -> TestResult {
    run_serial(|| {
        let kp = generate_keypair()?;
        let batch: Vec<&[u8]> = Vec::new();
        let signature = sign_for(&kp, &batch)?;
...
#[test]
fn guardian_042_empty_batch_signature_rejects_single_empty_transaction_batch() -> TestResult {
    run_serial(|| {
        let kp = generate_keypair()?;
        let empty_batch: Vec<&[u8]> = Vec::new();
...
#[test]
fn guardian_043_single_empty_transaction_signature_rejects_empty_batch() -> TestResult {
    run_serial(|| {
        let kp = generate_keypair()?;
        let empty_batch: Vec<&[u8]> = Vec::new();
        let single_empty: Vec<&[u8]> = vec![b"".as_slice()];
...
#[test]
fn guardian_044_single_zero_byte_transaction_differs_from_empty_transaction() -> TestResult {
    run_serial(|| {
        let kp = generate_keypair()?;
...
#[test]
fn guardian_045_same_bytes_different_allocations_verify() -> TestResult {
    run_serial(|| {
        let kp = generate_keypair()?;
        let tx_a = b"guardian-allocation-a".to_vec();
        let tx_b = b"guardian-allocation-b".to_vec();
...
#[test]
fn guardian_046_same_concatenated_bytes_different_transaction_boundaries_reject() -> TestResult {
    run_serial(|| {
        let kp = generate_keypair()?;
...
#[test]
fn guardian_047_signature_for_three_leaves_verifies_fourth_duplicate_last_leaf() -> TestResult {
    run_serial(|| {
        let kp = generate_keypair()?;
...
#[test]
fn guardian_048_signature_for_five_leaves_verifies_sixth_duplicate_last_leaf() -> TestResult {
    run_serial(|| {
        let kp = generate_keypair()?;
        let five: Vec<&[u8]> = vec![
            b"a".as_slice(),
...
#[test]
fn guardian_049_signature_for_single_leaf_rejects_two_duplicate_leaves() -> TestResult {
    run_serial(|| {
        let kp = generate_keypair()?;
...
#[test]
fn guardian_050_signature_for_three_leaves_rejects_fourth_different_leaf() -> TestResult {
    run_serial(|| {
        let kp = generate_keypair()?;
        let three: Vec<&[u8]> = vec![b"a".as_slice(), b"b".as_slice(), b"c".as_slice()];
...
#[test]
fn guardian_051_middle_signature_byte_flip_is_rejected() -> TestResult {
    run_serial(|| {
        let kp = generate_keypair()?;
...
#[test]
fn guardian_052_last_signature_byte_flip_is_rejected() -> TestResult {
    run_serial(|| {
        let kp = generate_keypair()?;
...
#[test]
fn guardian_053_reversed_exact_length_signature_is_rejected() -> TestResult {
    run_serial(|| {
        let kp = generate_keypair()?;
...
#[test]
fn guardian_054_rotated_exact_length_signature_is_rejected() -> TestResult {
    run_serial(|| {
        let kp = generate_keypair()?;
...
#[test]
fn guardian_055_drop_first_signature_byte_and_pad_end_is_rejected() -> TestResult {
    run_serial(|| {
        let kp = generate_keypair()?;
...
#[test]
fn guardian_056_add_leading_zero_and_drop_last_signature_byte_is_rejected() -> TestResult {
    run_serial(|| {
        let kp = generate_keypair()?;
...
#[test]
fn guardian_057_split_and_duplicate_signature_fragment_is_rejected() -> TestResult {
    run_serial(|| {
        let kp = generate_keypair()?;
...
#[test]
fn guardian_058_signature_from_key_a_rejects_key_b_even_for_same_public_batch() -> TestResult {
    run_serial(|| {
...
#[test]
fn guardian_059_two_keypairs_signatures_verify_only_with_own_keys() -> TestResult {
    run_serial(|| {
        let key_a = generate_keypair()?;
        let key_b = generate_keypair()?;
...
#[test]
fn guardian_060_manual_verify_empty_context_succeeds_but_one_byte_context_fails() -> TestResult {
    run_serial(|| {
        let kp = generate_keypair()?;
...
#[test]
fn guardian_061_batch_root_matches_manual_verification_root() -> TestResult {
    run_serial(|| {
        let kp = generate_keypair()?;
        let txs = tx_vectors("guardian-root-match", 10);
        let batch = tx_refs(&txs);
        let signature = sign_for(&kp, &batch)?;
        let root = guardian_batch_root(&batch)?;
...
#[test]
fn guardian_062_property_reordering_each_adjacent_pair_rejects() -> TestResult {
    run_serial(|| {
        let kp = generate_keypair()?;
        let original_txs = tx_vectors("guardian-swap", 7);
        let original_batch = tx_refs(&original_txs);
        let signature = sign_for(&kp, &original_batch)?;
...
#[test]
fn guardian_063_property_each_signature_probe_byte_flip_rejects() -> TestResult {
    run_serial(|| {
        let kp = generate_keypair()?;
...
#[test]
fn guardian_064_property_signature_invalid_lengths_near_boundary_reject() -> TestResult {
    run_serial(|| {
        let kp = generate_keypair()?;
...
#[test]
fn guardian_065_sign_and_verify_transaction_at_exact_small_power_of_two_counts() -> TestResult {
    run_serial(|| {
        let kp = generate_keypair()?;

        for count in [1_usize, 2, 4, 8, 16, 32] {
            let txs = tx_vectors("guardian-power-two", count);
            let batch = tx_refs(&txs);
            let signature = sign_for(&kp, &batch)?;
...
#[test]
fn guardian_066_sign_and_verify_transaction_at_odd_counts() -> TestResult {
    run_serial(|| {
        let kp = generate_keypair()?;

        for count in [3_usize, 5, 7, 9, 15, 17] {
            let txs = tx_vectors("guardian-odd-count", count);
            let batch = tx_refs(&txs);
            let signature = sign_for(&kp, &batch)?;
...
#[test]
fn guardian_067_large_valid_item_at_one_kib_succeeds() -> TestResult {
    run_serial(|| {
        let kp = generate_keypair()?;
...
#[test]
fn guardian_068_many_empty_transactions_succeed() -> TestResult {
    run_serial(|| {
        let kp = generate_keypair()?;
...
#[test]
fn guardian_069_many_identical_nonempty_transactions_succeed() -> TestResult {
    run_serial(|| {
        let kp = generate_keypair()?;
...
#[test]
fn guardian_070_many_identical_transactions_reject_when_one_value_changes() -> TestResult {
    run_serial(|| {
        let kp = generate_keypair()?;
...
#[test]
fn guardian_071_manual_merkle_root_changes_when_batch_boundaries_change() -> TestResult {
    run_serial(|| {
        let first: Vec<&[u8]> = vec![b"ab".as_slice(), b"cd".as_slice()];
        let second: Vec<&[u8]> = vec![b"a".as_slice(), b"bcd".as_slice()];

        let first_root = guardian_batch_root(&first)?;
        let second_root = guardian_batch_root(&second)?;
...
#[test]
fn guardian_072_manual_merkle_root_empty_batch_differs_from_single_empty_transaction() -> TestResult
{
    run_serial(|| {
        let empty_batch: Vec<&[u8]> = Vec::new();
        let single_empty: Vec<&[u8]> = vec![b"".as_slice()];

        let empty_root = guardian_batch_root(&empty_batch)?;
        let single_empty_root = guardian_batch_root(&single_empty)?;
...
#[test]
fn guardian_073_signature_for_batch_with_trailing_empty_tx_rejects_without_trailing_empty_tx()
-> TestResult {
    run_serial(|| {
        let kp = generate_keypair()?;
        let with_trailing_empty: Vec<&[u8]> =
...
#[test]
fn guardian_074_signature_for_batch_without_trailing_empty_tx_rejects_with_trailing_empty_tx()
-> TestResult {
    run_serial(|| {
        let kp = generate_keypair()?;
        let without_trailing_empty: Vec<&[u8]> = vec![b"a".as_slice(), b"b".as_slice()];
...
#[test]
fn guardian_075_adversarial_signature_duplicate_full_frame_rejects_by_length() -> TestResult {
    run_serial(|| {
        let kp = generate_keypair()?;
...
#[test]
fn guardian_076_adversarial_signature_missing_first_byte_rejects_by_length() -> TestResult {
    run_serial(|| {
        let kp = generate_keypair()?;
...
#[test]
fn guardian_077_adversarial_signature_missing_last_byte_rejects_by_length() -> TestResult {
    run_serial(|| {
        let kp = generate_keypair()?;
        let batch: Vec<&[u8]> = vec![b"missing-a".as_slice(), b"missing-b".as_slice()];
        let signature = sign_for(&kp, &batch)?;
...
#[test]
fn guardian_078_load_test_sign_verify_sixty_four_transaction_batch() -> TestResult {
    run_serial(|| {
        let kp = generate_keypair()?;
        let txs = tx_vectors("guardian-load-64", 64);
        let batch = tx_refs(&txs);
        let signature = sign_for(&kp, &batch)?;
...
#[test]
fn guardian_079_load_test_multiple_keys_sign_same_batch() -> TestResult {
    run_serial(|| {
        let txs = tx_vectors("guardian-multi-key-load", 16);
        let batch = tx_refs(&txs);
...
#[test]
fn guardian_080_load_test_many_batches_with_distinct_roots_verify() -> TestResult {
    run_serial(|| {
        let kp = generate_keypair()?;

        for round in 0..16 {
            let txs = tx_vectors("guardian-distinct-root-load", round + 1);
            let batch = tx_refs(&txs);
            let signature = sign_for(&kp, &batch)?;
            let root = guardian_batch_root(&batch)?;
...
#[test]
fn guardian_081_signature_for_seven_leaves_verifies_eighth_duplicate_last_leaf() -> TestResult {
    run_serial(|| {
        let kp = generate_keypair()?;
...
#[test]
fn guardian_082_signature_for_seven_leaves_rejects_eighth_different_leaf() -> TestResult {
    run_serial(|| {
        let kp = generate_keypair()?;
        let seven: Vec<&[u8]> = vec![
            b"a".as_slice(),
...
#[test]
fn guardian_083_signature_for_nine_leaves_verifies_tenth_duplicate_last_leaf() -> TestResult {
    run_serial(|| {
        let kp = generate_keypair()?;
        let nine: Vec<&[u8]> = vec![
            b"n0".as_slice(),
...
#[test]
fn guardian_084_signature_for_nine_leaves_rejects_tenth_different_leaf() -> TestResult {
    run_serial(|| {
        let kp = generate_keypair()?;
        let nine: Vec<&[u8]> = vec![
            b"n0".as_slice(),
...
#[test]
fn guardian_085_seven_leaf_root_equals_eight_leaf_root_when_last_is_duplicate() -> TestResult {
    run_serial(|| {
        let seven: Vec<&[u8]> = vec![
            b"a".as_slice(),
            b"b".as_slice(),
        ...
        assert_eq!(
            guardian_batch_root(&seven)?,
            guardian_batch_root(&eight_with_duplicate_last)?
        );
...
#[test]
fn guardian_086_nine_leaf_root_equals_ten_leaf_root_when_last_is_duplicate() -> TestResult {
    run_serial(|| {
        let nine: Vec<&[u8]> = vec![
            b"n0".as_slice(),
            b"n1".as_slice(),
        ...
        assert_eq!(
            guardian_batch_root(&nine)?,
            guardian_batch_root(&ten_with_duplicate_last)?
        );
...
#[test]
fn guardian_087_largest_policy_allowed_single_item_signs_and_verifies() -> TestResult {
    run_serial(|| {
        let kp = generate_keypair()?;
        let valid_len =
...
#[test]
fn guardian_088_largest_policy_allowed_single_item_rejects_one_byte_change() -> TestResult {
    run_serial(|| {
        let kp = generate_keypair()?;
        let valid_len =
            GlobalConfiguration::MAX_ITEM_BYTES.min(GlobalConfiguration::MAX_TOTAL_BATCH_BYTES);
...
#[test]
fn guardian_089_first_sixty_four_signature_bytes_zeroed_rejects() -> TestResult {
    run_serial(|| {
        let kp = generate_keypair()?;
...
#[test]
fn guardian_090_last_sixty_four_signature_bytes_zeroed_rejects() -> TestResult {
    run_serial(|| {
        let kp = generate_keypair()?;
...
#[test]
fn guardian_091_exact_length_all_max_byte_signature_rejects() -> TestResult {
    run_serial(|| {
        let kp = generate_keypair()?;
...
#[test]
fn guardian_092_exact_length_alternating_signature_rejects() -> TestResult {
    run_serial(|| {
        let kp = generate_keypair()?;
...
#[test]
fn guardian_093_empty_batch_root_equals_single_dummy_marker_transaction_root() -> TestResult {
    run_serial(|| {
        let empty_batch: Vec<&[u8]> = Vec::new();
        let dummy_marker_batch: Vec<&[u8]> = vec![b"remzar_empty_block_mint".as_slice()];

        assert_eq!(
            guardian_batch_root(&empty_batch)?,
            guardian_batch_root(&dummy_marker_batch)?
        );
...
#[test]
fn guardian_094_empty_batch_signature_verifies_single_dummy_marker_transaction() -> TestResult {
    run_serial(|| {
        let kp = generate_keypair()?;
        let empty_batch: Vec<&[u8]> = Vec::new();
        let dummy_marker_batch: Vec<&[u8]> = vec![b"remzar_empty_block_mint".as_slice()];
...
#[test]
fn guardian_095_single_dummy_marker_signature_verifies_empty_batch() -> TestResult {
    run_serial(|| {
        let kp = generate_keypair()?;
        let empty_batch: Vec<&[u8]> = Vec::new();
        let dummy_marker_batch: Vec<&[u8]> = vec![b"remzar_empty_block_mint".as_slice()];
...
#[test]
fn guardian_096_dummy_marker_signature_rejects_dummy_marker_with_extra_byte() -> TestResult {
    run_serial(|| {
        let kp = generate_keypair()?;
        let dummy_marker_batch: Vec<&[u8]> = vec![b"remzar_empty_block_mint".as_slice()];
...
#[test]
fn guardian_097_property_sampled_signature_byte_flips_reject_across_full_length() -> TestResult {
    run_serial(|| {
        let kp = generate_keypair()?;
...
#[test]
fn guardian_098_property_each_transaction_removal_rejects_for_ten_item_batch() -> TestResult {
    run_serial(|| {
        let kp = generate_keypair()?;
        let txs = tx_vectors("guardian-remove-each", 10);
        let batch = tx_refs(&txs);
        let signature = sign_for(&kp, &batch)?;
...
#[test]
fn guardian_099_load_test_sign_verify_one_hundred_twenty_eight_transaction_batch() -> TestResult {
    run_serial(|| {
        let kp = generate_keypair()?;
        let txs = tx_vectors("guardian-load-final-128", 128);
        let batch = tx_refs(&txs);
        let signature = sign_for(&kp, &batch)?;
        let root = guardian_batch_root(&batch)?;
...
#[test]
fn guardian_100_load_test_twelve_keys_sign_and_verify_same_empty_batch() -> TestResult {
    run_serial(|| {
        let batch: Vec<&[u8]> = Vec::new();
